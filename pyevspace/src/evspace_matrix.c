...
#ifndef EVSPACE_MATRIX_C
#define EVSPACE_MATRIX_C
...
static void
multiply_matrix_vector_states(const double* mat, const double* vec, 
...
multiply_matrix_vector(const EVSpace_Matrix* mat, const EVSpace_Vector* vec)
{
	...
	multiply_matrix_vector_states(Matrix_DATA(mat), Matrix_DATA(vec), ans, 1);

	PyObject* rtn = vector_steal_array(ans, &EVSpace_VectorType);
	if (!rtn)
		free(ans);
...
matrix_add(EVSpace_Matrix* lhs, PyObject* rhs)
{
	if (Matrix_Check(lhs) && Matrix_Check(rhs))
		return add_matrix_matrix(lhs, (EVSpace_Matrix*)rhs);
...
{
	if (Matrix_Check(lhs) && Matrix_Check(rhs))
		return subtract_matrix_matrix(lhs, (EVSpace_Matrix*)rhs);
...
matrix_multiply(EVSpace_Matrix* self, PyObject* arg)
{
	if (Matrix_Check(self)) {
		if (Vector_Check(arg))
			return multiply_matrix_vector(self, (EVSpace_Vector*)arg);

		if (Matrix_Check(arg))
			return multiply_matrix_matrix(self, (EVSpace_Matrix*)arg);
		...
		if (PyNumber_Check(arg)) {
			...
			return multiply_matrix_scalar(self, scalar);
		}
	}
...
{
	if (Matrix_Check(self))
		return capsule_matrix_negative(self);
...
matrix_determinate(PyObject* Py_UNUSED(_), PyObject* const* args, Py_ssize_t size)
{
	...
	double det = capsule_determinate((EVSpace_Matrix*)args[0]);
...
matrix_transpose(PyObject* Py_UNUSED(_), PyObject* const* args, Py_ssize_t size)
{
	...
	return capsule_transpose((EVSpace_Matrix*)args[0]);
}


#endif // EVSPACE_MATRIX_C
